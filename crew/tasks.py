...
WRITER_SYSTEM = (
    ...
    "number that was given to you. ALSO CRITICAL: placeholder values like "
    "'Not publicly available', 'N/A', or empty strings/lists are DATA "
    "MARKERS, not sentences -- you must never copy one of these literally "
    "into a paragraph of prose (e.g. writing 'Not publicly available. The "
    "market for...' as if it were a sentence). Wherever a field you need is "
    "one of these markers, substitute the exact phrase 'Data unavailable' "
    "for that part instead, in every single section, with no exceptions -- "
    "this applies identically to Overview, What They Do, Business Model, "
    "and every other section, not just some of them."
)
...
## Overview
(1-2 sentences from product_summary + market_opportunity. IMPORTANT: if
product_summary is "Not publicly available", empty, or missing, do NOT
write that placeholder text as if it were a sentence -- write exactly
"Data unavailable" for this section instead, then continue with
market_opportunity separately if that field does have real content.)
...
infrastructure, cloud, or architecture detail to fill the gap. If
product_summary is ALSO "Not publicly available" or empty, write exactly
"Data unavailable" here, same as in Overview.)
...
missing/empty/"Not publicly available", write exactly "Data unavailable"
for that part -- this is a strict find-and-replace, applied the same way
in every section of this report, not just some of them. Never let the
literal string "Not publicly available" appear anywhere in the rendered
markdown output -- it is a data marker from the JSON, not report text.
Keep Quick Facts as a markdown table.
    """